...
module.exports = {
  ...
  rules: {
    ...
    // @fix 禁止不必要的布尔类型转换，比如 !! 或 Boolean
    'no-extra-boolean-cast': 'error',

    // @fix 禁止函数表达式中出现多余的括号，比如 let foo = (function () { return 1 })
    'no-extra-parens': [
      'error',
      'functions'
    ],
    ...
    // 禁止 function 和 var 声明出现在嵌套的语句块中
    ...
    // 禁止使用特殊空白符（比如全角空格），除非是出现在字符串、注释、正则表达式或模版字符串中
    ...
    // @fix 必须使用 === 或 !==，禁止使用 == 或 !=，与 null 比较时除外
    'eqeqeq': [
      'error',
      'always',
      {
    ...
    // @fix 禁止 if 语句中 return 语句之后有 else 块
    ...
    // @fix new, typeof 等后面必须有空格，++, -- 等禁止有空格
    'space-unary-ops': [
      'error',
      {
        words: true,
